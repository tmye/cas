...
//                $ss = (($employe->getSalary()   / $nbreJourTravail) / $donnees["dailyTime"] ) * ($donnees["dailyTime"]);
                $ss = $donnees["salTotal"] ;
            } else if ($type == "2" || $type == 2) {
                if ($taux == 0)
                    $ss = 0;
                else
                    $ss = ((($employe->getSalary() ) / $nbreJourTravail) / $taux) * $donnees["quota_total"] / 60;
...
//                    $ss = ((($employe->getSalary() * 12) / 52) / $taux) * $donnees["quota_1_4"];
            ...
            if ($type == "2" or $type == 2) {
                ...
                if ($t == "2") {
                    ...
                    $data2 = array(
                        array("Temps (H)", $donnees["tpa"], round($donnees["tpr"], 2), round($donnees["tpd"], 2), $donnees["lost_time"], round($donnees["tpa"] + $donnees["tpr"] + $donnees["tpd"] + $donnees["lost_time"], 2), $permission_lost_time, round($donnees["tempsBonus"] * (-1), 2)),
                    );
                    $data3 = array(
                        array("Somme (FCFA) ", round($donnees["spa"], 2), round($donnees["spr"], 2), round($donnees["spd"], 2), round($donnees["spAuth"], 2), round($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"], 2), $taux == 0 ? 0 : round((($employe->getSalary() * 12) / 52) / $taux * $permission_lost_time, 2), round($donnees["sommeArgentBonus"] * (-1), 0)),
                    );
                    $data4 = array(
                        array(utf8_decode("Net à payer sans bonus"), round($ss - ($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"]), 2)),
                    );
                ...
                if ($t == "2") {
                    ...
                    $data2 = array(
                        array("Temps (H)", $donnees["tpa"], round($donnees["tpr"], 2), round($donnees["tpd"], 2), $donnees["lost_time"], round($donnees["tpa"] + $donnees["tpr"] + $donnees["tpd"] + $donnees["lost_time"], 2), round($permission_lost_time,2)),
                    );
                    $data3 = array(
                        array("Somme (FCFA)", round($donnees["spa"], 2), round($donnees["spr"], 2), round($donnees["spd"], 2), round($donnees["spAuth"], 2), round($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"], 2), $taux == 0 ? 0 : round((($employe->getSalary() * 12) / 52) / $taux * $permission_lost_time, 2)),
                    );
                    $data4 = array(
                        array(utf8_decode("Net à payer sans bonus"), round($ss - ($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"]), 2)),
                    );
                    ...
                    $data2 = array(
                        array("Temps (H)", $donnees["tpa"], round($donnees["tpr"], 2), round($donnees["tpd"], 2), $donnees["lost_time"], round($donnees["tpa"] + $donnees["tpr"] + $donnees["tpd"] + $donnees["lost_time"], 2), round($permission_lost_time,2), round($donnees["tempsBonus"] * (-1), 2)),
                    );
                    $data3 = array(
                        array("Somme (F)", round($donnees["spa"], 2), round($donnees["spr"], 2), round($donnees["spd"], 2), round($donnees["spAuth"], 2), round($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"], 2),  round(($donnees["salPerHour"]  * $permission_lost_time), 2), round($donnees["sommeArgentBonus"] * (-1), 0)),
                    );
                    $data4 = array(
                        array(utf8_decode("Net à payer sans bonus"), round($ss - ($donnees["spa"] + $donnees["spr"] + $donnees["spd"] + $donnees["spAuth"]), 2)),
                    );
