<?php
//Todo begin refonte, set layout and at least 10pages
...
class DefaultController extends StatsController
{
    ...
    /**
     * @Route("/testok",name="testok")
     */
    public function testAction(Request $request)
    {

        return $this->render("cas/permission_preview.html.twig");
    }
...
//        print_r($theHeight);
        ...
        $permissionType = [1=>'Repos maladie', 2=>'Congés', 3=>'Autre'];
        ...
        $pdf->Ln('10');
        $pdf->Write(7, "Je vous prie de m'accorder une permission pour m'absenter du ".$permission->getDateFrom()->format("Y-m-d")." ".$permission->getTimeFrom()." au ".$permission->getDateTo()->format("Y-m-d")." ".$permission->getTimeTo()." pour me permettre de (". utf8_decode($permissionType[$permission->getType()]).utf8_decode(").
...
//        echo "res array  ".count($result_array)."<br/>";
    ...
    public function generateExcelAction(Request $request)
    {
       ...
       /* $permissionDayStyleArray = [
        ...
        // legende ;
        $sheet->setCellValue('D3', 'LEGENDE');
        $spreadsheet->getActiveSheet()->getStyle('D3')->applyFromArray($boldStyle);
        // different colors
        $spreadsheet->getActiveSheet()->getStyle('D4')->applyFromArray($lessLateDayStyleArray);
        $sheet->setCellValue('E4', 'Retard Léger');

        $spreadsheet->getActiveSheet()->getStyle('D5')->applyFromArray($midLateDayStyleArray);
        $sheet->setCellValue('E5', 'Retard Moyen');

        $spreadsheet->getActiveSheet()->getStyle('D6')->applyFromArray($veryLateDayStyleArray);
        $sheet->setCellValue('E6', 'Retard Grave');

        $spreadsheet->getActiveSheet()->getStyle('D7')->applyFromArray($not_supposed_to_come_styleArray);
        $sheet->setCellValue('E7', 'Weekend');

        $spreadsheet->getActiveSheet()->getStyle('D8')->applyFromArray($permissionDayStyleArray);
        $sheet->setCellValue('E8', 'Permission');
...
//            $sheet->setCellValue('A' . ($nextNameCellNumber + 2),  "MASCA");
            ...
            for ($cpt = 3; $cpt <= $days; $cpt++) {
                ...
                // si une journee inclue dans une permission, on l'encercle de vert
                if ($this->dateIsUnderPermission($employe, $nowTime)) {
                    $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber -1).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 5))
                        ->applyFromArray($permissionDayStyleArray);
                }

                // firs time
                switch($this->checkDaySeverityBeginHour($emp, $nowTime, $his)) {
                    case 0:
                        $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber+2).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 2))
                            ->applyFromArray($lessLateDayStyleArray);
                        break;
                    case 1:
                        $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber+2).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 2))
                            ->applyFromArray($midLateDayStyleArray);
                        break;
                    case 2:
                        $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber+2).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 2))
                            ->applyFromArray($veryLateDayStyleArray);
                        break;
                }

                // leaving time
                switch($this->checkDaySeverityLastHour($emp, $nowTime, $his)) {
                    case 0:
                        $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber+5).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 5))
                            ->applyFromArray($lessLateDayStyleArray);
                        break;
                    case 1:
                        $sheet->getStyle($verticalCellsTab[$cpt] . '' . ($nextNameCellNumber+5).':'.$verticalCellsTab[$cpt] . '' . ($nextNameCellNumber + 5))
    ...
    private function checkIfDayHasPermission($nowTime)
    {
        ...
        // check if there is a permission start date that comes before this nowtime

        // if no, pass, if yes, check if end date of the permission is today or after today

        return false;
    }

    private function checkDaySeverityBeginHour($emp, $nowTime, $his)
    {
        // arrive
        // pause
        // finPause
        ...
        $employee = $this->EmployeeRepo()->find($emp);
        if ($employee != null) {
            $wh_obj = $this->WorkingHourRepo()->find($employee->getWorkingHour());
            $code = $wh_obj->getWorkingHour();
            $wh_array = json_decode($code, true);

            $dayOfTheWeek = date('w', $nowTime); // 0 sunday, 6 saturday
            $this->info("now time ".date("Y-m-d H:i:s", $nowTime));
            ...
            if (($dayOfTheWeek == 0 && $wh_array[$dayLibelle][0]["type"] ==  "null" )) {
                return -1;
            } else {

                if ($wh_array[$dayLibelle][0]["type"] != null) {

                    $difference = $this->compareStartHour($wh_array[$dayLibelle][0]["beginHour"], $his["arrive"]);
    ...
    private function checkDaySeverityLastHour($emp, $nowTime, $his)
    {
        // arrive
        // pause
        // finPause
        ...
        if ($employee != null) {
            ...
            if (($dayOfTheWeek == 0 && $wh_array[$dayLibelle][0]["type"] ==  "null" )) {
                ...
                if ($wh_array[$dayLibelle][0]["type"] != null) {
                    ...
                    $difference = $this->compareEndHour($wh_array[$dayLibelle][0]["endHour"], $his["depart"]);
    ...
    private function compareStartHour($beginHour, $arrivee)
    {
...
//        echo $beginHour."<br/>";
        if ($beginHour == null || $arrivee == null)
            return 0;

        $settingTime = intval(explode(":", $beginHour)[0])*60+intval(explode(":", $beginHour)[1]);

        $realTime = intval(explode(":", $arrivee)[0])*60+intval(explode(":", $arrivee)[1]);

        return $realTime-$settingTime < 0 ? 0 : $realTime - $settingTime;
    }

    private function compareEndHour($beginHour, $arrivee)
    {
